...
public class InvoicesWindow extends JDialog {
    ...
    public InvoicesWindow(JFrame parent) {
        super(parent, "Invoices Management", true);
    ...
    // Metodo helper per convertire timestamp/string a Date
    private Date parseDate(ResultSet rs, String columnName) throws SQLException {
        try {
            // Prima prova a leggerlo come timestamp
            ...
            try {
                ...
                if (dateStr != null && !dateStr.isEmpty()) {
                    // Se è un numero (timestamp), convertilo
                    if (dateStr.matches("\\d+")) {
                        long timestampMillis = Long.parseLong(dateStr);
                        return new Date(timestampMillis);
                    }
                    // Altrimenti prova a parsarlo come data formattata
                    return dateFormat.parse(dateStr);
                }
            } catch (Exception ex) {
                // Se tutto fallisce, ritorna null
                return null;
            }
        }
        return null;
        ...
        try {
            ...
            try (Statement stmt = conn.createStatement();
                ...
                while (rs.next()) {
                    ...
                    // FIX: Usa il metodo helper per gestire il parsing della data
                    Date date = parseDate(rs, "data");
                    if (date != null) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(query)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    Date date = parseDate(rs, "data");
                    if (date == null) {
                        date = new Date(); // Fallback alla data corrente
                    }
