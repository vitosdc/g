...
public class InvoicePDFGenerator {
    ...
    public void generateAndSave(Component parent) {
        // DEBUG: Stampa informazioni dell'invoice per debug
        System.out.println("=== DEBUG INVOICE INFO ===");
        System.out.println("Invoice ID: " + invoice.getId());
        System.out.println("Invoice Number: " + invoice.getNumero());
        System.out.println("Invoice Date: " + invoice.getData());
        System.out.println("Customer ID: " + customer.getId());
        System.out.println("Customer Nome: " + customer.getNome());
        System.out.println("Customer Cognome: " + customer.getCognome());
        System.out.println("==========================");
        ...
        // FIXED: Nome file più significativo e robusto
        // 1. Numero fattura (sempre presente)
        ...
        // 2. Nome cliente (prova cognome, poi nome, poi default)
        String customerName = "";
        ...
        // Sanitizza il nome cliente
        customerName = customerName.replaceAll("[^a-zA-Z0-9._-]", "_");
        
        // 3. Data fattura (prova data fattura, poi data corrente)
        String dateString = "";
        try {
            ...
            // Se la formattazione fallisce, usa timestamp
            dateString = String.valueOf(System.currentTimeMillis() / 1000);
        }
        
        // 4. Costruisci nome file finale
        String defaultName = String.format("Invoice_%s_%s_%s.pdf", 
            invoiceNumber,
            customerName,
            dateString);
            
        System.out.println("Generated filename: " + defaultName); // Debug
    ...
    private void generatePDF(File outputFile) throws IOException {
        try (PDDocument document = new PDDocument()) {
            ...
            try (PDPageContentStream contentStream = new PDPageContentStream(document, page)) {
                ...
                // Draw header
                yPosition = drawHeader(contentStream, yPosition, page, document);
                yPosition -= 30f;
                ...
                // Draw footer
                drawFooter(contentStream, yPosition, page);
            }
    ...
    private float drawHeader(PDPageContentStream contentStream, float yPosition, PDPage page, PDDocument document) throws IOException {
    ...
    private float drawItemsTable(PDPageContentStream contentStream, float yPosition) throws IOException {
        float tableWidth = 595f - 2 * MARGIN; // A4 width is 595 points
        ...
        for (InvoiceItem item : items) {
            // FIXED: Controlli per valori null
            String code = item.getProdottoCodice();
            String name = item.getProdottoNome();
            
            String[] rowData = {
                code != null ? code : "",
                name != null ? truncateText(name, 25) : "Product N/A",
    ...
    private void drawTableRow(PDPageContentStream contentStream, float x, float y, float[] columnWidths, 
        ...
        for (int i = 0; i < data.length && i < columnWidths.length; i++) {
            // FIXED: Controllo per valori null
            String text = data[i];
            if (text == null) {
                text = ""; // Sostituisci null con stringa vuota
            }
            
            // Controlla se il testo è vuoto prima di disegnarlo
    ...
    private void drawFooter(PDPageContentStream contentStream, float yPosition, PDPage page) throws IOException {
        // Payment terms
        contentStream.beginText();
        contentStream.setFont(new PDType1Font(Standard14Fonts.FontName.HELVETICA_BOLD), 10f);
