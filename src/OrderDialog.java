...
public class OrderDialog extends JDialog {
    ...
    // METODO CORRETTO PER EVITARE RICORSIONE INFINITA
    private void updateTotals() {
        if (updatingTotals) return; // Previene la ricorsione
        
        updatingTotals = true;
        try {
            double total = 0;
            for (int i = 0; i < itemsTableModel.getRowCount(); i++) {
                // CORRETTO IL CASTING DEI VALORI
                ...
                // Gestione sicura del casting per quantity
                ...
                // Gestione sicura del casting per price
                if (priceObj instanceof Double) {
                    price = (Double) priceObj;
                } else {
                    String priceStr = priceObj.toString().replace(",", ".");
                    price = Double.parseDouble(priceStr);
                }
                
                double itemTotal = quantity * price;
                
                // Aggiorna il totale della riga - questo ora non causerà ricorsione
                    ...
                    try (PreparedStatement pstmt = conn.prepareStatement(detailQuery)) {
                        for (int i = 0; i < itemsTableModel.getRowCount(); i++) {
                            ...
                            int quantity = quantityObj instanceof Integer ? 
                                (Integer)quantityObj : Integer.parseInt(quantityObj.toString());
